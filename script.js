...
(function enforceStrictMobileWithQR() {
   ...
   document.addEventListener("visibilitychange", () => {
      if (document.hidden) clearInterval(window.sparkleTimer);
      else window.sparkleTimer = setInterval(generateSparkles, 4800);
   });
})();
...
function initAfterLoad() {
   ...
   setTimeout(() => {
      startRevealStory();
   }, 1400); // slightly > cardEnter duration
}
...
window.sparkleTimer = setInterval(generateSparkles, 4800);
...
function startRevealStory() {
   ...
   // 🔑 HARD RESET reveal styles (fixes Android bug)
   ...
   function revealNext() {
      ...
      // Start dancer animation after stagger
